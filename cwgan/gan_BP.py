...
def Generator(n_samples, y = None, noise = None):
    ...
    output = lib.ops.linear.Linear('Generator.Input', noise_shape[1], g_h * g_w * 4 * DIM, noise)
    if MODE == 'wgan':
...
def Discriminator(inputs, y = None):
    ...
    output = lib.ops.linear.Linear('Discriminator.Output', 1*93*4*DIM, 1, output)
...
fixed_noise = tf.constant(np.random.normal(size=(BATCH_SIZE, 93)).astype('float32'))
