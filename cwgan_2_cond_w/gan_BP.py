...
flags.DEFINE_string("cond", "00000", "conditional")
...
sample_dir='./joint3_2' + '_' + FLAGS.cond + '_' + FLAGS.concat+'/samples'
test_dir = './joint3_2' + '_' + FLAGS.cond + '_' + FLAGS.concat+'/test'
cost_dir = './joint3_2' + '_' + FLAGS.cond + '_' + FLAGS.concat+'/cost'
list_dir = './joint3_2' + '_' + FLAGS.cond + '_' + FLAGS.concat
...
def Classifier(inputs, y = None, prop = None, Reuse=None):
    # default :"NDHWC"
    output = tf.reshape(inputs, [-1, 23, 26, 31, 1])
    # 1st concat
    '''
    if (FLAGS.cond[4] == '1'):
        if (FLAGS.concat == '0'):
            ...
            if Reuse:
                with tf.variable_scope(tf.get_variable_scope(),reuse= Reuse):
                    w = tf.get_variable('Classifier.w4', [23, 26, 31, y_dim, 15], initializer=tf.truncated_normal_initializer(stddev=0.02))
                    output = concat_2(output, yb, w)
            else:
                w = tf.get_variable('Classifier.w4', [23, 26, 31, y_dim, 15],
                                    initializer=tf.truncated_normal_initializer(stddev=0.02))
                output = concat_2(output, yb, w)
