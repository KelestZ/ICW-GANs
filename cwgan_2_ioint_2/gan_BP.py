...
sample_dir='./gen_images' + '_' + FLAGS.cond + '_' + FLAGS.concat+'/samples'
test_dir = './gen_images' + '_' + FLAGS.cond + '_' + FLAGS.concat+'/test'
cost_dir = './gen_images' + '_' + FLAGS.cond + '_' + FLAGS.concat+'/cost'
list_dir = './gen_images' + '_' + FLAGS.cond + '_' + FLAGS.concat
...
def Generator(n_samples, y = None, noise = None, Reuse = None):
    ...
    if(FLAGS.cond[1] =='1'):
        if(FLAGS.concat == '0'):
            ...
            '''
            yb2_ = lib.ops.linear.Linear('Generator.yb2_', y_dim, 2 * 2 * 2 * 3, y)
            yb2_ = tf.tanh(yb2_)
            yb2_ = tf.reshape(yb2_, [-1, 2, 2, 2, 3])
            output = concat([output, yb2_], 4)

            '''
            if Reuse:
                with tf.variable_scope(tf.get_variable_scope(), reuse = True):
                    w = tf.get_variable('Generator.w1', [ 2, 2, 2, y_dim, 3],initializer=tf.truncated_normal_initializer(stddev=0.02))
    ...
    if (FLAGS.cond[2] == '1'):
        if (FLAGS.concat == '0'):
            ...
            '''
            yb3_ = lib.ops.linear.Linear('Generator.yb3_', y_dim, 3 * 4 * 4 * 3, y)
            yb3_ = tf.tanh(yb3_)
            yb3_ = tf.reshape(yb3_, [-1, 3, 4, 4, 3])
            output = concat([output, yb3_], 4)

            '''
            if Reuse:
                with tf.variable_scope(tf.get_variable_scope(), reuse = True):
                    w = tf.get_variable('Generator.w2', [3, 4, 4, y_dim, 3],
    ...
    if (FLAGS.cond[3] == '1'):
        if (FLAGS.concat == '0'):
            ...
            '''
            yb4_ = lib.ops.linear.Linear('Generator.yb4_', y_dim, 6* 7* 8 * 3, y)
            yb4_ = tf.tanh(yb4_)
            yb4_ = tf.reshape(yb4_, [-1, 6, 7, 8, 3])
            output = concat([output, yb4_], 4)
            '''
            if Reuse:
                with tf.variable_scope(tf.get_variable_scope(), reuse = True):
                    w = tf.get_variable('Generator.w3', [6, 7, 8, y_dim, 3],initializer=tf.truncated_normal_initializer(stddev=0.02))
                    output = concat_2(output, yb, w)
            else:
                w = tf.get_variable('Generator.w3', [6, 7, 8, y_dim, 3],initializer=tf.truncated_normal_initializer(stddev=0.02))
                output = concat_2(output, yb, w)
    ...
    if (FLAGS.cond[4] == '1'):
        if (FLAGS.concat == '0'):
            ...
            '''
            yb5_ = lib.ops.linear.Linear('Generator.yb5_', y_dim, 12* 13* 16* 3, y)
            yb5_ = tf.tanh(yb5_)
            yb5_ = tf.reshape(yb5_, [-1, 12, 13, 16, 3])
            output = concat([output, yb5_], 4)
            '''
            if Reuse:
                with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                    w = tf.get_variable('Generator.w4', [12, 13, 16, y_dim, 3],
                                        initializer=tf.truncated_normal_initializer(stddev=0.02))
                    output = concat_2(output, yb, w)
            else:
                w = tf.get_variable('Generator.w4', [12, 13, 16, y_dim, 3],
...
def Discriminator(inputs, y = None, prop = None, Reuse=None):
    ...
    if (FLAGS.cond[4] == '1' and flag==1):
        if (FLAGS.concat == '0'):
            ...
            '''
            yb5_ = lib.ops.linear.Linear('Discriminator.yb5_', y_dim,  23* 26* 31 * 3, y)
            yb5_ = tf.reshape(yb5_, [-1, 23, 26, 31, 3])
            yb5_ = tf.tanh(yb5_)
            output = concat([output, yb5_], 4)
            '''
            ...
            if Reuse:
                with tf.variable_scope(tf.get_variable_scope(),reuse= Reuse):
                    w = tf.get_variable('Discriminator.w4', [23, 26, 31, y_dim, 3], initializer=tf.truncated_normal_initializer(stddev=0.02))
                    output = concat_2(output, yb, w)
            else:
                w = tf.get_variable('Discriminator.w4', [23, 26, 31, y_dim, 3],
                                    initializer=tf.truncated_normal_initializer(stddev=0.02))
                output = concat_2(output, yb, w)
    ...
    if (FLAGS.cond[3] == '1' and flag==1):
        if (FLAGS.concat == '0'):
            ...
            '''
            yb4_ = lib.ops.linear.Linear('Discriminator.yb4_', y_dim,  12* 13* 16 * 3, y)
            yb4_ = tf.reshape(yb4_, [-1, 12, 13, 16, 3])
            yb4_ = tf.tanh(yb4_)
            output = concat([output, yb4_], 4)
            '''
            if Reuse:
                with tf.variable_scope(tf.get_variable_scope(), reuse= Reuse):
                    w = tf.get_variable('Discriminator.w3', [12, 13, 16, y_dim, 3], initializer=tf.truncated_normal_initializer(stddev=0.02))
                    output = concat_2(output, yb, w)
            else:
                w = tf.get_variable('Discriminator.w3', [12, 13, 16, y_dim, 3],
                                    initializer=tf.truncated_normal_initializer(stddev=0.02))
                output = concat_2(output, yb, w)
    ...
    if (FLAGS.cond[2] == '1'  and flag==1):
        if (FLAGS.concat == '0'):
            ...
            '''
            yb3_ = lib.ops.linear.Linear('Discriminator.yb3_', y_dim, 6* 7* 8 * 3, y)
            ...
            if Reuse:
                with tf.variable_scope(tf.get_variable_scope(), reuse=Reuse):
                    w = tf.get_variable('Discriminator.w2', [6, 7, 8, y_dim, 3],
                                initializer=tf.truncated_normal_initializer(stddev=0.02))
                    output = concat_2(output, yb, w)
            else:
                w = tf.get_variable('Discriminator.w2', [6, 7, 8, y_dim, 3],
    ...
    if (FLAGS.cond[1] == '1' and flag==1):
        if (FLAGS.concat == '0'):
            ...
            '''
            yb2_ = lib.ops.linear.Linear('Discriminator.yb2_', y_dim, 3* 4* 4 * 3, y)
            ...
            if Reuse:
                with tf.variable_scope(tf.get_variable_scope(), reuse=Reuse):
                    w = tf.get_variable('Discriminator.w1', [3, 4, 4, y_dim, 3],
                                initializer=tf.truncated_normal_initializer(stddev=0.02))
                    output = concat_2(output, yb, w)
            else:
                w = tf.get_variable('Discriminator.w1', [3, 4, 4, y_dim, 3],
    ...
    if (FLAGS.cond[0] == '1'  and flag==1):
        ...
        output = lib.ops.linear.Linear('Discriminator.5', output_shape[-1], 2*2*2*8 * DIM, output)
        '''
...
disc_real = Discriminator(real_data, y, prop, None) #clas_real
disc_fake = Discriminator(fake_data, y, prop, True) #clas_fake
...
if MODE == 'wgan':
    ...
    output1 = Discriminator(interpolates, y, prop, True)
...
train_gen, dev_gen = lib.BrainPedia.load_ckpt_data(BATCH_SIZE, list_dir) # load_mnist(BATCH_SIZE)# test_gen
...
with tf.Session() as session:
    ...
    for iteration in xrange(ITERS):
        ...
        if iteration > 0:
            # for i in xrange(3):
            _,_, g_loss = session.run([classifier_train_op,gen_train_op, gen_cost],feed_dict={real_data: _data, y: targets, prop: np.array(temp_prop)})
            #classifier_train_op
        ...
        if iteration % 100 == 0 and iteration != 0:
            ...
            for images, tags in dev_gen():  # image and targets
                ...
                preds_,_dev_disc_cost, _dev_g_loss = session.run(
                    [preds,disc_cost, gen_cost],
            ...
            print('iteration',iteration,'acc:%.3f' % (ac))
        ...
        lib.plot.tick()
        if (iteration > 18000):#18000):  # 11000
