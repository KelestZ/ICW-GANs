...
def upsample_vectorized(vecdata):
    ...
    # get mask
    inmask, smallmasker ,affine = down_mask(2.0)
    #23, 26, 31
    outshape = tuple([26,31,23])#[13 ,15 ,11])
...
# 这里很多错误啊，如果结果有提高就不改了，如果没有的话，重新跑一下
def upsample_voxels(masker,voxeldata):
    ...
    # RESAMPLE to full brain space，不回去了，直接是scaling 2倍的
    braindata = resample_img(voxeldata, target_affine=masker.get_affine(),target_shape=masker.shape, interpolation='continuous')
