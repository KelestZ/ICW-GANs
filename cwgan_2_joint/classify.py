...
def Discriminator(inputs):
    # default :"NDHWC"
    output = tf.reshape(inputs, [-1, 26, 31, 23, 1])
    ...
    output = lib.ops.linear.Linear('Discriminator.5',output.get_shape().as_list()[-1], 128, output)
...
preds = tf.to_int32(tf.round(tf.nn.softmax(logits))) # maybe -->0
    ...
    for iteration in xrange(ITERS):
        ...
        if (iteration >= 6000):
            ...
            if (accuracy_score(tag_list, pre_list) > ac):
                ...
                f = open('./cm_gen_2.pkl','wb')#cm_real_0 #cm_0
                pkl.dump([tag_list,pre_list],f)
                f.close()
