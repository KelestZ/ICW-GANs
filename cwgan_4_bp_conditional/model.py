...
class CIWGAN(object):
    def __init__(self, sess, MODE='wgan-gp', DIM=64, BATCH_SIZE =50, LAMBDA = 10, ITERS =200000,
                ...
                cost_dir=None, test_dir = None, sample_dir = None):
    ...
    def build_model(self):
        ...
        self.fake_data = self.Generator(self.BATCH_SIZE, self.y, None)

        self.disc_real = self.Discriminator(self.real_data, self.y, None)
        self.disc_fake = self.Discriminator(self.fake_data, self.y, True)
        ...
        if MODE=='wgan-gp':
            ...
            gradients = tf.gradients(self.Discriminator(interpolates, self.y, True), [interpolates])[0]
    ...
    def train(self):
        train_gen, dev_gen = lib.BrainPedia.load_BrainPedia_test(BATCH_SIZE)
        ...
        for iteration in xrange(ITERS):
            ...
            if np.mod(iteration, 5) == 0 and iteration != 0:
                print('[INFO] Save checkpoint...')
                self.save(self.checkpoint_dir, iteration)

            if (iteration > 15000):  # 18000):  # 11000
