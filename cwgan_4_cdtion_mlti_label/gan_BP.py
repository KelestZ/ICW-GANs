...
sample_dir = './nn_multi_label' + '_' + FLAGS.cond + '_' + FLAGS.concat+'/samples'
test_dir = './nn_multi_label' + '_' + FLAGS.cond + '_' + FLAGS.concat+'/test'
cost_dir = './nn_multi_label' + '_' + FLAGS.cond + '_' + FLAGS.concat+'/cost'
data_list_dir='./nn_multi_label' + '_' + FLAGS.cond + '_' + FLAGS.concat
...
#clas_params = lib.params_with_name('Classifier')
if MODE == 'wgan':
    ...
    learning_rate3 = tf.train.exponential_decay(
        ...
        staircase=True)
    '''
    classify_train_op = tf.train.AdamOptimizer(
        learning_rate=learning_rate3,
        beta1=0.5,
        beta2=0.9
    ).minimize(closs, var_list=clas_params)
    '''
...
train_gen, dev_gen = lib.BrainPedia.load_cross(BATCH_SIZE)#, test_gen, data_list_dir) # load_mnist(BATCH_SIZE)#
...
def save_test_img(frame):
    ...
    for i, label in enumerate(ran):
        test_labels[i] = cvt_45_19_binary[label]
