...
flags.DEFINE_string("cond", "1111", "conditional")
...
sample_dir = './test' + '_' + FLAGS.cond + '_' + FLAGS.concat+'/samples'
test_dir = './test' +'_' + FLAGS.cond + '_' + FLAGS.concat+'/test'
cost_dir = './test' + '_' + FLAGS.cond + '_' + FLAGS.concat+'/cost'
data_list_dir='./test' + '_' + FLAGS.cond + '_' + FLAGS.concat
...
def Discriminator(inputs, y = None, prop = None, Reuse=None):
    ...
    #output_c = lib.ops.linear.Linear('Discriminator.COutput', 128, y_dim, output)
...
def Classifier(inputs):
    ...
    output = lib.ops.conv3d.Conv3D('Classifier.C1', output.get_shape().as_list()[-1], DIM, output, stride=2)
    #output = lib.ops.batchnorm.Batchnorm('Discriminator.CBN0', [0, 1, 2, 3], output)

    output = LeakyReLU(output)
    #output = tf.nn.avg_pool3d(output, ksize=[1, 2, 2, 2, 1], strides=[1, 2, 2, 2, 1],padding='SAME', data_format=None)
    # print('pool1:', output.shape)
    output = lib.ops.conv3d.Conv3D('Classifier.C2', output.get_shape().as_list()[-1], 2 * DIM, output, stride=2)
    #output = lib.ops.batchnorm.Batchnorm('Discriminator.CBN1', [0, 1, 2, 3], output)

    output = LeakyReLU(output)

    #output = tf.nn.avg_pool3d(output, ksize=[1, 2, 2, 2, 1], strides=[1, 2, 2, 2, 1],padding='SAME', data_format=None)
    # print('pool2:', output.shape)
    output = lib.ops.conv3d.Conv3D('Classifier.C3', output.get_shape().as_list()[-1], 4 * DIM, output, stride=2)
    #output = lib.ops.batchnorm.Batchnorm('Discriminator.CBN3', [0, 1, 2, 3], output)
    ...
    # add one linear layer into the model
    output = lib.ops.linear.Linear('Classifier.C5', 2 * 2 * 2 * 4 * DIM, 128, output)
    #output = lib.ops.batchnorm.Batchnorm('Generator.CBN4', [0], output)

    #out_c = lib.ops.linear.Linear('Classifier.C5', 128, 2, output)
    output = lib.ops.linear.Linear('Classifier.COutput', 128, y_dim, output)

    return output#,tf.nn.sigmoid(out_c)
...
closs = batch_closs_real + batch_closs_fake
...
if MODE == 'wgan':
    ...
    # disc_fake_fetch = tf.reduce_mean(disc_fake)
    # disc_real_fetch = tf.reduce_mean(disc_real)
    #gen_cost = -tf.reduce_mean(disc_fake) + batch_closs_fake
    ...
    classifier_train_op = tf.train.AdamOptimizer(
        ...
        beta1=0.5,
        beta2=0.9
    ).minimize(closs, var_list=class_params)
...
train_gen, dev_gen= lib.BrainPedia.load_cross(BATCH_SIZE)#, data_list_dir) # load_mnist(BATCH_SIZE)#
#, test_gen
'''
choose_pool = []
for i in range(45):
...
with tf.Session() as session:
    ...
    for iteration in xrange(ITERS):
        ...
        if iteration > 0:
            _,_disc_cost, _, g_loss = session.run([classifier_train_op,disc_cost, gen_train_op, gen_cost],
                                                                   feed_dict={real_data: _data, y: targets, prop: np.array(temp_prop)})
            #print('Classifier REAL & FAKE',closs_real_,closs_fake_)
            #print('DISC REAL & FAKE', real_disc, fake_disc)
            #classifier_train_op
        ...
        '''
        '''
        if (iteration > 10000):#18000):  # 11000
