...
def Generator(n_samples, y = None, noise = None):
    ...
    output = lib.ops.deconv2d.Deconv2D('Generator.2', output.get_shape().as_list()[1], [BATCH_SIZE, g_h, 185, 2 * DIM],
                                       filter_size, output)
    if MODE == 'wgan':
        output = lib.ops.batchnorm.Batchnorm('Generator.BN2', [0, 2, 3], output)
    output = LeakyReLU(output)
    # output = output[:,:,:1,:91]
    # print('g_shape1:',output)
    # 3rd concat
    output = conv_cond_concat(output, yb)
    # 2 * DIM
    output = lib.ops.deconv2d.Deconv2D('Generator.3', output.get_shape().as_list()[1], [BATCH_SIZE, g_h, 369, DIM],
                                       filter_size, output)
    ...
    #4th concat
    #output = conv_cond_concat(output, yb)
    output = lib.ops.deconv2d.Deconv2D('Generator.5', DIM, [BATCH_SIZE, g_h, 737, 1], filter_size,
                                       output)  # 364*50 = 18200
...
def Discriminator(inputs, y = None):
    ...
    output = lib.ops.linear.Linear('Discriminator.Output0', 1 * 93 * 4 * DIM, 10, output)

    output = lib.ops.linear.Linear('Discriminator.Output',10, 1, output)
...
''' 
sample_yFile = open('sample_y.txt','rb')
labels_temp = pkl.load(sample_yFile)
sample_yFile.close()

for i in range(len(ch)):
    print('[INFO]ch[i]:',ch[i])
    fixed_labels[len(labels_temp)+i:] = labels_temp[ch[i]]

'''

train_gen = lib.BrainPedia.load(BATCH_SIZE) # load_mnist(BATCH_SIZE)#

'''
fixed_labels = np.zeros([BATCH_SIZE,y_dim]).astype(np.float32)
slice = sample_y.shape[0]

idx = BATCH_SIZE//len(sample_y)
for i in range(idx):
    fixed_labels[i*len(sample_y):(1+i)*len(sample_y)] = sample_y.copy()

mod = BATCH_SIZE - idx * (len(sample_y))
if mod != 0:
    fixed_labels[-mod:] = sample_y[:mod].copy()

a = np.sum(np.sum(fixed_labels,1),0)
'''
