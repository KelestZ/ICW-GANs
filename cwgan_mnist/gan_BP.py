...
def Generator(n_samples, y = None, noise = None):
    ...
    if noise is None:
        noise = tf.random_normal([n_samples, g_w])

    yb = tf.reshape(y, [BATCH_SIZE,  1, 1, y_dim])
    # 1st concat
    noise = concat([noise, y], 1)
    noise_shape = noise.get_shape().as_list()
    #print('n_shape:', noise_shape)
    #print('w_shape:',noise_shape[1], g_h * g_w * 4 * DIM)
    # input_dim（shape[1]）, output_dim(shape[1])
    output = lib.ops.linear.Linear('Generator.Input', noise_shape[1], 4*4*4*DIM, noise)
    if MODE == 'wgan':
    ...
    output = lib.ops.deconv2d.Deconv2D('Generator.2', output.get_shape().as_list()[1], [BATCH_SIZE, 8, 8, 2 * DIM], filter_size, output)
    if MODE == 'wgan':
    ...
    #output = conv_cond_concat(output, yb)
    # 2 * DIM
    output = lib.ops.deconv2d.Deconv2D('Generator.3', output.get_shape().as_list()[1], [BATCH_SIZE, 14, 14, DIM], filter_size, output)
    if MODE == 'wgan':
        output = lib.ops.batchnorm.Batchnorm('Generator.BN3', [0,2,3], output)
    output = LeakyReLU(output)

    output = lib.ops.deconv2d.Deconv2D('Generator.5', DIM, [BATCH_SIZE, 28, 28, 1], filter_size, output) # 364*50 = 18200
...
def Discriminator(inputs, y = None):
    output = tf.reshape(inputs, [-1, 1, 784])
...
with tf.Session() as session:
    ...
    for iteration in xrange(ITERS):
        ...
        if iteration > 0:
            _ = session.run(gen_train_op,feed_dict={y: targets})
