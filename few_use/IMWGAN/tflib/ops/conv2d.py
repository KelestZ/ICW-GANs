...
def Conv2D(name, input_dim, output_dim, filter_size, inputs, he_init=True, mask_type=None, stride=1, weightnorm=None, biases=True, gain=1.):
    ...
    with tf.name_scope(name) as scope:
        ...
        '''
        try:
            result = tf.nn.conv2d(inputs, filters, output_shape=output_shape, strides=[1, 2, 2, 1], padding='SAME')
                # Support for verisons of TensorFlow before 0.7.0
        except AttributeError:
            result = tf.nn.deconv2d(inputs, filters, output_shape=output_shape, padding='SAME',
                                     strides=[1, 2, 2, 1])
        '''
        # stride = 2
        result = tf.nn.conv2d(
