...
# 没改完，class里的我都不知道要改什么 #
class DCGAN(object):
    def __init__(self, sess, input_height=28, input_width=28, crop=True,
                 batch_size=64, sample_num=28, output_height=28, output_width=28,
        ...
        #self.sample_inputs,self.sample_labels = self.load_BrainPedia_test(imageFile, labelFile)
        self.sample_inputs = self.data_X[0:self.sample_num]
        self.sample_labels = self.data_y[0:self.sample_num]
    ...
    def build_model(self):
        ...
        # D: 对的都是1，错的都是0
        # G: 标签都是1
        self.d_loss_real = tf.reduce_mean(sigmoid_cross_entropy_with_logits(self.D_logits, tf.ones_like(self.D)))#tf.reduce_mean(
        self.d_loss_fake = tf.reduce_mean(sigmoid_cross_entropy_with_logits(self.D_logits_, tf.zeros_like(self.D_)))#tf.reduce_mean(
    ...
    def train(self, config):
        ...
        decay_steps = int(batch_idxs * config.num_epochs_per_decay)
        ...
        d_train_op = tf.contrib.layers.optimize_loss(
            ...
            optimizer="RMSProp",
            clip_gradients=5.0,
            learning_rate_decay_fn=learning_rate）
        ...
        if config.dataset == 'mnist' or config.dataset == 'BrainPedia':
            ...
            sample_inputs = self.sample_inputs[0:self.sample_num]
            sample_labels = self.sample_labels[0:self.sample_num]# ch] #self.data_y[
            #print(sample_inputs.shape,sample_labels.shape)
        else:
        ...
        for epoch in xrange(0,config.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                batch_z = np.random.uniform(-0.5, 0.5, [config.batch_size, self.z_dim]) \
                    .astype(np.float32)
                ...
                if (config.dataset == 'mnist'):
                    ...
                    self.writer.add_summary(summary_str, counter)
                    # Run g_optim more than once to make sure that d_loss does not go to zero (different from paper)
                    for k in range(3):
                        # Update G network
                        _, summary_str = self.sess.run([g_optim, self.g_sum],
                                                   feed_dict={
                ...
                if np.mod(counter, 100) == 1:
                    if config.dataset == 'mnist':
                        samples, d_loss, g_loss,lr = self.sess.run(
                            [self.sampler, self.d_loss, self.g_loss,self.learning_rate],
                            feed_dict={
                                self.z: sample_z,
                        ...
                        #print("[INFO] Sample labels ",sample_labels)
                        print("Epoch: [%2d] [%4d/%4d] time: %4.4f, d_loss: %.8f, g_loss: %.8f" \
                              % (epoch, idx, batch_idxs,
                                 time.time() - start_time, errD_fake + errD_real, errG))
                        print("[Sample] d_loss: %.8f, g_loss: %.8f" % (d_loss, g_loss))
                        ...
                        ct = 0
                        # 到时候要试试看能不能打开
                        # print('[INFO] len(sample): ',len(sample))
                        # 这里似乎必须写batch_size啊
                        for i in samples.reshape([self.batch_size, -1]):
                            ...
                            filename = './{}/train_{:02d}_{:02d}_{:02d}.nii.gz'.format(config.sample_dir, epoch, idx,ct)
    ...
    def discriminator(self, image, y=None, reuse=False):
        with tf.variable_scope("discriminator") as scope:
            ...
            if not self.y_dim:
                ...
                h1 = lrelu((conv2d(h0, self.df_dim * 2, name='d_h1_conv')))# self.d_bn1
                ...
                h1 = lrelu((conv2d(h0, self.df_dim + self.y_dim, name='d_h1_conv')))# 3 self.d_bn1
                h1 = tf.reshape(h1, [self.batch_size, -1])
                h1 = concat([h1, y], 1)
                '''
                ### 在这里加了一层
                h4 = lrelu(self.d_bn3(conv2d(h1, self.df_dim + self.y_dim, name='d_h4_conv')))
                h4 = tf.reshape(h4, [self.batch_size, -1])
                h4 = concat([h4, y], 1)
                '''
                h2 = lrelu((linear(h1, self.dfc_dim, 'd_h2_lin'))) #self.d_bn2
                h2 = concat([h2, y], 1)
    ...
    def sampler(self, z, y=None):
        with tf.variable_scope("generator") as scope:
            ...
            if not self.y_dim:
                ...
                h0 = tf.nn.relu((linear(z, self.gfc_dim, 'g_h0_lin')))#self.g_bn0
                h0 = concat([h0, y], 1)

                h1 = tf.nn.relu((linear(h0, self.gf_dim * 2 * s_h4 * s_w4, 'g_h1_lin')))#self.g_bn1

                h1 = tf.reshape(h1, [self.batch_size, s_h4, s_w4, self.gf_dim * 2])
                h1 = conv_cond_concat(h1, yb)

                h2 = tf.nn.relu((deconv2d(h1, [self.batch_size, s_h2, s_w2, self.gf_dim * 2], name='g_h2')))#self.g_bn2
                h2 = conv_cond_concat(h2, yb)
    ...
    def critic_mlp(self, img, y=None, reuse=False):
        with tf.variable_scope("discriminator") as scope:
            ...
            # 一层 linear 1024
            h0 = lrelu(self.d_bn1(linear(x, 128, 'd_h0_lin')))
            h0 = tf.reshape(h0, [self.batch_size, -1])
            h0 = concat([h0, y], 1)

            # 两层 linear  64
            h1 = lrelu( self.d_bn2(linear(h0, 64, 'd_h1_lin')))
            h1 = tf.reshape(h1, [self.batch_size, -1])
            h1 = concat([h1, y], 1)
            '''
            # 三层 linear 64
    ...
    def generate_mlp(self,z, y=None):
        with tf.variable_scope("generator") as scope:
            ...
            # 一层 linear
            z = concat([z, y], 1)
            h0 = tf.nn.relu(self.g_bn0(linear(z, 64 , 'g_h0_lin')))
            h0 = tf.reshape(h0, [self.batch_size, -1])
            h0 = concat([h0, y], 1)

            # 两层 linear

            h1 = tf.nn.relu(self.g_bn1(linear(h0, 128, 'g_h1_lin')))
            h1 = tf.reshape(h1, [self.batch_size, -1])
            h1 = concat([h1, y], 1)
            '''
            # 三层 linear
    ...
    def load_mnist(self):
        ...
        save_images(x1, image_manifold_size(x1.shape[0]),
                    './{}/train_{}.png'.format('samples','training_data' ))

        for i in range(6000):
            ran = np.random.choice(64,1)
            test_x.append(x1[ran])
            test_y.append(y1[ran])

        test_x = np.array(test_x).reshape((6000, 28, 28, 1))
    ...
    def load_BrainPedia_test(self, imageFile, labelFile):
        x = []
        y = []
        count = 0
        ...
        for i, label in enumerate(y):
            y_vec[i, y[i]] = 1.0
        print('[INFO] xshape',x[0].shape)
        print('[INFO] y_vec.shape ',y_vec[0].shape)
