...
class DCGAN(object):
    def __init__(self, sess, input_height=28, input_width=28, crop=True,
                 ...
                 gfc_dim=1024, dfc_dim=1024, c_dim=9, dataset_name='mnist',
                 input_fname_pattern='*.png', checkpoint_dir=None, sample_dir=None, change = False):
    ...
    def train(self, config):
        ...
        d_optim = tf.train.GradientDescentOptimizer(self.learning_rate)\
            .minimize(self.d_loss, var_list=self.d_vars, global_step=self.global_step)

            #, beta1=config.beta1) \
        g_optim = tf.train.GradientDescentOptimizer(0.01) \
    ...
    def load_mnist(self):
        ...
        save_images(x1, image_manifold_size(x1.shape[0]),
                    './{}/train.png'.format('samples'))

        for i in range(64000//len(x1)):
            for k in range(len(x1)):
                test_x.append(x1[k])
                test_y.append(y1[k])

        print('y_shape: ', len(test_y))
        #print('x_shape: ', test_x.shape)
        test_x = np.array(test_x).reshape((64000, 28, 28, 1))
        test_y = np.array(test_y).reshape((64000, self.y_dim))
