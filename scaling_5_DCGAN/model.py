...
class DCGAN(object):
    def __init__(self, sess, input_height=10, input_width=12, crop=True,
                 batch_size=64, sample_num=64, output_height=10, output_width=12,
        ...
        if self.dataset_name == 'mnist':
            ...
            imageFile = base_dir + 'outbraindata_5.pkl'
            ...
            for idx in xrange(0, batch_idxs):
                ...
                if np.mod(counter, 100) == 1:
                    ...
                    ####  改动的地方  ####
                        ...
                        ct = 0
                        # 到时候要试试看能不能打开
                        for i in samples.reshape(self.batch_size, self.output_height, self.output_width, self.c_dim):
                            # pkl.dump(i, outputFile)
                            temp = upsample_voxels(i)
    ...
    def load_BrainPedia(self, imageFile, labelFile):
        ...
        for i in labelpkl.keys():
            x.append(imgpkl[i].get_data())
            y.append(labelpkl[i])
            count += 1

        # 维度不知道
        x, y = shuffle(np.array(x).reshape((6573, 10, 12, 9)).astype(np.float64),
